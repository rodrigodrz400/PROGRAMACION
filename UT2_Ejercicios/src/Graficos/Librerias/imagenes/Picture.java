package Graficos.Librerias.imagenes;
...
public class Picture implements Shape {
	 ...
	 */
	public int getWidth() {
		return (int) Math.round((image == null ? 0 : image.getWidth()) + 2
				* xGrow);
	}
	...
	public int getHeight() {
		return (int) Math.round((image == null ? 0 : image.getHeight()) + 2
				* yGrow);
	}
	...
	public int[][] getGrayLevels() {
		...
		for (int i = 0; i < grayLevels.length; i++)
			for (int j = 0; j < grayLevels[i].length; j++) {
				...
				grayLevels[i][j] = (int) (0.2989 * ((rgb >> 16) & 0xFF)
						+ 0.5866 * ((rgb >> 8) & 0xFF) + 0.1144 * (rgb & 0xFF));
			}
		return grayLevels;
	}

	public Picture(int[][] grayLevels) {
		image = new BufferedImage(grayLevels[0].length, grayLevels.length,
	...
	public String toString() {
		return "Picture[x=" + getX() + ",y=" + getY() + ",width=" + getWidth()
				+ ",height=" + getHeight() + ",source=" + source + "]";
	}
	 ...
	 *            the y-coordinate (row) of the pixel
	 * @param the
	 *            color of the pixel at the given row and column
	 */
	public void setColorAt(int x, int y, Color color) {
		if (image == null || x < 0 || x >= image.getWidth() || y < 0
			...
			image.setRGB(x, y,
					((int) color.getRed()) * 65536 + ((int) color.getGreen())
							* 256 + (int) color.getBlue());
			Canvas.getInstance().repaint();
		}
	}
	 ...
	 * 
	 * @param dx
	 *            the amount by which to move in x-direction
	 * @param dy
	 ...
	 * 
	 * @param dw
	 *            the amount by which to resize the width on each side
	 * @param dw
	 *            the amount by which to resize the height on each side
	 */
